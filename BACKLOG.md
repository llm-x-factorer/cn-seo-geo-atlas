...
- [x] **`exp-cn-llm-platform-4-monetization-form-monitor`** —— ⭐⭐⭐ 6 大平台 **4 形态平台变现监控** ✅ **2026-05-04 完成实验设计 + T0 基线**：[`experiments/exp-cn-llm-platform-4-monetization-form-monitor.md`](./experiments/exp-cn-llm-platform-4-monetization-form-monitor.md)（273 行 / 4-6 小时初始搭建 / ¥0 现金）。**T0 核心发现**：形态 1 订阅制 3/6 已上线（豆包 / 文心 / Kimi）/ 形态 2 电商佣金 2/6 已链接（豆包 → 抖音商店 / 千问 → 淘宝）/ 形态 3 自建品牌方监测后台 **0/6 上线**（最高优先级监测点）/ 形态 4 API 收费 4-5/6 已商业化（豆包定价 doubao-seed-2.0-pro 3.2/16 元每百万 tokens 已公开）。监测节奏：季度全面 + 月度告警 + 一次性事件即时触发。后续运营任务见下条。
- [ ] **`tech-cn-llm-platform-4-form-quarterly-report`** —— 平台 4 形态变现季度报告（每 3 个月输出 1 篇 / 更新 T0 基线 / 触发仓库 DS 假说重审）。第一份报告窗口 2026-08。
