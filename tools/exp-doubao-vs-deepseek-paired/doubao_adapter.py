"""
豆包 Web 端适配器。

⚠️ 所有 selector 标注 `# TODO: pilot-verify` 的位置，必须在 pilot run 后人工核查
并改为 `# verified YYYY-MM-DD`，方可用于主实验。

UI 持续迭代，selector 失效时直接修复并 PR。
"""

from __future__ import annotations

import asyncio
import time
from typing import Any

from playwright.async_api import Page, TimeoutError as PlaywrightTimeoutError


# === 待 pilot 验证的 selector ===
# 修改后请把注释改为 `# verified YYYY-MM-DD`
SELECTORS = {
    # 输入框（聊天框）
    "input_textarea": "textarea[placeholder*='发消息']",  # TODO: pilot-verify

    # 发送按钮
    "send_button": "button[type='submit']",  # TODO: pilot-verify

    # 答案容器（最后一条 AI 消息）
    "answer_container": "[class*='message'][class*='assistant']:last-of-type",  # TODO: pilot-verify

    # "正在生成"指示符（出现 = 还在生成）
    "generating_indicator": "[class*='generating'], [class*='thinking']",  # TODO: pilot-verify

    # 引用源卡片 / 列表
    "citation_list": "[class*='citation'], [class*='reference'], [class*='source']",  # TODO: pilot-verify

    # 引用源中的链接
    "citation_link": "a[href]",  # TODO: pilot-verify

    # 联网搜索开关（如有）
    "online_search_toggle": "button[aria-label*='联网'], button[aria-label*='搜索']",  # TODO: pilot-verify
}


URL = "https://www.doubao.com/chat"
PLATFORM_NAME = "doubao"


async def collect(
    page: Page,
    query: str,
    config: dict[str, Any],
    *,
    online_search: bool = True,
) -> dict[str, Any]:
    """
    在豆包 Web 端运行一次 query，返回采集结果。

    返回字段（与 deepseek_adapter.collect 保持一致）：
    {
        "platform": "doubao",
        "platform_url": ...,
        "query_text": ...,
        "online_search_enabled": ...,
        "model_id": ...,                 # 如 UI 显示
        "platform_version": ...,         # 如 UI 显示
        "raw_response_text": ...,
        "raw_response_html": ...,        # 答案区域的 HTML，便于离线再分析
        "citation_sources": [
            {"rank": 1, "title": ..., "url": ...},
            ...
        ],
        "screenshot_path": ...,
        "response_time_ms": ...,
        "errors": [...]                  # 任何采集异常
    }
    """
    timeout_s = config.get("timeout_seconds", 60)
    errors: list[str] = []

    result: dict[str, Any] = {
        "platform": PLATFORM_NAME,
        "platform_url": URL,
        "query_text": query,
        "online_search_enabled": online_search,
        "model_id": None,
        "platform_version": None,
        "raw_response_text": None,
        "raw_response_html": None,
        "citation_sources": [],
        "screenshot_path": None,
        "response_time_ms": None,
        "errors": errors,
    }

    started_at = time.time()

    try:
        await page.goto(URL, wait_until="domcontentloaded", timeout=30_000)

        # 等待输入框可见
        await page.wait_for_selector(
            SELECTORS["input_textarea"], state="visible", timeout=15_000
        )

        # （可选）切换联网搜索
        # 默认假定平台 UI 默认开启，仅在配置要求关闭时操作
        # 实际开关交互需 pilot 验证后实现
        if not online_search:
            errors.append(
                "online_search=False but adapter does not yet implement toggle off"
            )

        # 输入 query 并发送
        await page.fill(SELECTORS["input_textarea"], query)
        await page.click(SELECTORS["send_button"])

        # 等待答案生成完成
        # 策略：循环轮询 generating_indicator，消失即视为生成结束
        deadline = time.time() + timeout_s
        last_text = ""
        stable_iters = 0
        while time.time() < deadline:
            await asyncio.sleep(1)
            try:
                indicator = await page.query_selector(SELECTORS["generating_indicator"])
                if indicator is None:
                    # 文本不再变化连续 3 次视为收敛
                    cur_text = await _get_answer_text(page)
                    if cur_text == last_text and cur_text:
                        stable_iters += 1
                        if stable_iters >= 3:
                            break
                    else:
                        stable_iters = 0
                    last_text = cur_text
            except Exception as e:
                errors.append(f"polling error: {e!r}")

        # 抽取答案文本与 HTML
        result["raw_response_text"] = await _get_answer_text(page)
        result["raw_response_html"] = await _get_answer_html(page)

        # 抽取引用源
        result["citation_sources"] = await _extract_citations(page)

        # 截图
        if config.get("screenshot_path"):
            await page.screenshot(
                path=config["screenshot_path"],
                full_page=True,
            )
            result["screenshot_path"] = config["screenshot_path"]

    except PlaywrightTimeoutError as e:
        errors.append(f"timeout: {e!r}")
    except Exception as e:
        errors.append(f"unexpected: {e!r}")

    result["response_time_ms"] = int((time.time() - started_at) * 1000)
    return result


async def _get_answer_text(page: Page) -> str:
    el = await page.query_selector(SELECTORS["answer_container"])
    if el is None:
        return ""
    return (await el.inner_text()).strip()


async def _get_answer_html(page: Page) -> str:
    el = await page.query_selector(SELECTORS["answer_container"])
    if el is None:
        return ""
    return await el.inner_html()


async def _extract_citations(page: Page) -> list[dict[str, Any]]:
    """
    豆包的引用源 UI 形态可能是：
    - 答案下方"引用源"小卡片（横滑列表）
    - 行内角标 [1] [2] 链接

    pilot 时确认实际 UI，必要时调整 selector。
    """
    citations: list[dict[str, Any]] = []

    list_el = await page.query_selector(SELECTORS["citation_list"])
    if list_el is None:
        return citations

    links = await list_el.query_selector_all(SELECTORS["citation_link"])
    for rank, link in enumerate(links, start=1):
        href = await link.get_attribute("href")
        title = (await link.inner_text()).strip()
        if href:
            citations.append(
                {
                    "rank": rank,
                    "title": title or None,
                    "url": href,
                }
            )

    return citations
